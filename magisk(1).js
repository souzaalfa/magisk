...
extractDate = function(line) {
    ...
    if (l.includes("profile") || l.includes("mcprofile")) {
  ...
  try {
    ...
    for (let line of lines) {
      ...
      if (proxyName) {
          ...
          if (dateInfo) {
              proxyLogs.push({ file: file, reason: "Proxy: " + proxyName, line: line.slice(0, 150), date: dateInfo.type + ": " + dateInfo.date });
          } else {
              proxyLogs.push({ file: file, reason: "Proxy: " + proxyName, line: line.slice(0, 150), date: "Detectado em log" });
          }
...
let html =
  ...
  (hashLogs["length"] > 0x0
    ? "\n  <h2 style=\"font-size:14px; color:#888; margin-left:4px\">HASHES DETECTADOS</h2>\n" +
      hashLogs.map((h) => {
              let proxyType = identifyProxy(h.value) || "Hash Identificado";
	          return `<div class="log-card" style="border-left: 4px solid #ff3b3b; background: #111; padding: 12px; border-radius: 12px; margin-bottom: 10px;">
	            ...
	            <div class="log-reason" style="font-size: 16px; color: #fff; margin-bottom: 4px; font-weight: 700;">${proxyType}</div>
	            ...
	            <div class="log-line" style="background: #000; border: 1px solid #222; margin-top: 10px; color: #666; font-size: 10px; padding: 8px; border-radius: 6px; overflow-x: auto;">${h.line}</div>
	          </div>`;
	      }).join("")
    : "") +
