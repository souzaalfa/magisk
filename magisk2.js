...
function identifyProxy(line) {
    // Lê apenas os 2 primeiros caracteres da linha
    let initials = line.substring(0, 2).toLowerCase();
    for (let p of proxyDatabase) {
        for (let pattern of p.patterns) {
            // Compara as iniciais com os padrões da tabela (usando os 2 primeiros caracteres do padrão também)
            if (pattern.toLowerCase().startsWith(initials)) return p.name;
        }
    }
    return null;
}
...
extractDate = function(line) {
    ...
    if (l.includes("mcprofile")) {
        ...
        else type = "PERFIL DETECTADO";
    }
    
    if (type === "") return null;
    
    let dateStr = dateMatch ? dateMatch[0] : "Sem data";
    return { type: type, date: dateStr };
};
...
for (let f of vmh_765d39["files"]) {
  try {
    ...
    for (let line of lines) {
      ...
      // Identifica o proxy apenas pelas 2 primeiras iniciais
      let proxyName = identifyProxy(line);
      
      // Se encontrou o proxy pelas iniciais, verifica se é um mcprofile e extrai a data e a hash
      if (proxyName) {
          let dateInfo = extractDate(line);
          if (dateInfo) {
              proxyDetected = true;
              let hashInfo = extractHash(line);
              let displayValue = "Data: " + dateInfo.date;
              if (hashInfo) {
                  displayValue += " | " + hashInfo.type + ": " + hashInfo.value;
              }
              ...
              hashLogs.push({ 
                  ...
                  type: dateInfo.type, 
                  value: displayValue, 
                  line: line.trim(), // Mostra a linha inteira para evitar falso positivo
                  proxyName: proxyName 
              });
          }
      }
...
let html =
  ...
  "\x0a\x0a" +
	  ...
	  (hashLogs["length"] > 0x0
	    ? "\n  <h2 style=\"font-size:14px; color:#888; margin-left:4px\">DETECÇÕES (mcprofile)</h2>\n" +
	      hashLogs.map((h) => {
	              let pName = h.proxyName || "Identificado";
		          return `<div class="log-card" style="border-left: 4px solid #ff3b3b; background: #111; padding: 12px; border-radius: 12px; margin-bottom: 10px;">
		            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
	                    <span class="log-file" style="color: #888; font-size: 10px;">${h.file}</span>
	                    <span class="tag" style="background: rgba(255,59,59,0.15); color: #ff3b3b; border: 1px solid rgba(255,59,59,0.3); font-weight: bold; font-size: 9px; padding: 2px 6px; border-radius: 4px;">${h.type}</span>
	                </div>
		            <div class="log-reason" style="font-size: 16px; color: #fff; margin-bottom: 4px; font-weight: 700;">${pName}</div>
	                <div style="font-size: 12px; color: #ff6b6b; font-family: monospace; word-break: break-all; opacity: 0.8; background: rgba(0,0,0,0.3); padding: 4px; border-radius: 4px;">${h.value}</div>
		            <div class="log-line" style="background: #000; border: 1px solid #222; margin-top: 10px; color: #666; font-size: 10px; padding: 8px; border-radius: 6px; overflow-x: auto; white-space: pre-wrap; word-break: break-all;">Log: ${h.line}</div>
		          </div>`;
			      }).join("")
		    : "") +
