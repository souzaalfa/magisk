...
for (let f of vmh_765d39["files"]) {
  try {
    ...
    for (let line of lines) {
      ...
      // Se encontrou o proxy pelas iniciais, verifica se é um mcprofile e extrai a data
      if (proxyName) {
          ...
          if (dateInfo) {
              ...
              hashLogs.push({ 
                  file: file, 
                  type: dateInfo.type, 
                  value: "Data: " + dateInfo.date, 
                  line: line.substring(0, 2), // Mostra apenas as 2 iniciais no log
                  proxyName: proxyName 
              });
          }
      }
...
let html =
  ...
  "<img id=\"logo\" src=\"https://files.manuscdn.com/user_upload_by_module/session_file/310519663816484294/MtMbwviJSLjkayEG.gif\" style=\"width: 45px; height: 45px; border-radius: 10px; border: 1px solid #333;\">" +
  "<div>" +
	    ...
	    ? "\n  <h2 style=\"font-size:14px; color:#888; margin-left:4px\">DETECÇÕES (mcprofile)</h2>\n" +
	      hashLogs.map((h) => {
		          ...
		          return `<div class="log-card" style="border-left: 4px solid #ff3b3b; background: #111; padding: 12px; border-radius: 12px; margin-bottom: 10px;">
		            ...
		            <div class="log-line" style="background: #000; border: 1px solid #222; margin-top: 10px; color: #666; font-size: 10px; padding: 8px; border-radius: 6px; overflow-x: auto;">Iniciais: ${h.line}</div>
		          </div>`;
			      }).join("")
		    : "") +
